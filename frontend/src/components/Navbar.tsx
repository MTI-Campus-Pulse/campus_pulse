...
//   const [searchQuery, setSearchQuery] = useState('')
//   const [isLoggedIn, setIsLoggedIn] = useState(false)
//   const [mounted, setMounted] = useState(false) // عشان نحل مشكلة الـ Hydration بدون تغيير شكل الكود
//   const router = useRouter()
//   const pathname = usePathname()

//   // التعديل الوحيد هنا عشان يلقط اللوجين واللوج أوت صح
//   useEffect(() => {
//     setMounted(true)
//     const checkAuth = () => {
//       if (typeof window !== 'undefined') {
//         const hasProfile = localStorage.getItem('userProfile') !== null
//         const isGuest = localStorage.getItem('isGuest') === 'true'
//         // هنا الشرط: لو فيه بروفايل ومش داخل كـ ضيف، يبقىLoggedIn
//         setIsLoggedIn(hasProfile && !isGuest)
//       }
//     }
...
//     checkAuth()
//     window.addEventListener('storage', checkAuth)
//     // ده بيخلي الناف بار يحس بأي تغيير بيحصل في الـ LocalStorage فوراً
...
//     const query = searchQuery.trim().toLowerCase()
//     if (!query) return
//     const matched = mockNews.find((news) => news.title.toLowerCase().includes(query))
//     if (matched) {
//       router.push(`/news/${matched.slug}`)
//     } else {
//       alert(`No exact match found for "${searchQuery}".`)
//     }
//     setSearchQuery('')
//     setIsOpen(false)
//   }

//   // الجزء ده عشان نضمن إن الزراير متظهرش غلط وقت التحميل (Hydration Fix)
...
//                 value={searchQuery}
//                 onChange={(e) => setSearchQuery(e.target.value)}
//                 className="pl-10 pr-4 h-10 bg-white/80 dark:bg-slate-800/80 border-slate-300 dark:border-slate-700 focus:border-indigo-500 rounded-full shadow-sm"
//               />
...
//       {/* Mobile menu (نفس الكود الأصلي بتاعك) */}
...
//                 value={searchQuery}
//                 onChange={(e) => setSearchQuery(e.target.value)}
//                 className="pl-10 pr-4 h-12 rounded-full shadow-sm w-full"
//               />
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      if (typeof window !== 'undefined') {
        const storedUser = localStorage.getItem('user') // only login sets this
